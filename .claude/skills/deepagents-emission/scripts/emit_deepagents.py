...
def emit_all(root: Path, ir_path: Path, output_dir: Path | None = None) -> dict:
    ...
    if mcp_env_vars:
        mcp_env_vars_block = "\n".join(
            ["# MCP-required environment variables (set per your .mcp.json)"]
            + [f"# {name}=" for name in mcp_env_vars]
        )
    ...
    common_vars = {
        ...
        "MCP_ENV_VARS_BLOCK": mcp_env_vars_block,
    ...
    # langgraph.json, .env.example, and bootstrap_ui.sh wire the default
    # langchain-ai/deep-agents-ui frontend. Always emitted — the UI is the
    # default frontend for every generated DeepAgents app.
    files_to_render = [
        ...
        ("langgraph.json.tmpl", app_dir / "langgraph.json"),
        ("env.example.tmpl", app_dir / ".env.example"),
        ("bootstrap_ui.sh.tmpl", app_dir / "bootstrap_ui.sh"),
    ]
