...
- Provider 표(Anthropic / OpenAI / Azure OpenAI / Bedrock / Vertex)와 `bootstrap_ui.sh` 사용법이 포함됨

### `langgraph.json.tmpl`
- 변수 없음. graph id는 항상 `deepagent` (langchain-ai/deep-agents-ui 기본값과 일치)

### `env.example.tmpl`
- `{{MODEL_DEFAULT}}`, `{{APP_NAME_DEFAULT}}` — config.py와 동일 값
- `{{MCP_ENV_VARS_BLOCK}}` — MCP env var를 `# NAME=` 형태로 주석 처리한 블록 (없으면 빈 문자열)
- Anthropic / OpenAI / Azure OpenAI / Bedrock / Vertex 프로바이더 블록을 모두 포함 (uncomment-to-use)

### `bootstrap_ui.sh.tmpl`
- 변수 없음. 셸 환경변수로 동작 제어: `LANGGRAPH_URL`, `AGENT_ID`, `DEEP_AGENTS_UI_REF`
- 출력 파일은 emit 시 chmod 0o755 부여
